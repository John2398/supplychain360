...
with DAG(
    ...
    extract_s3 = PythonOperator(
        task_id="extract_s3", python_callable=s3_ingestion.transfer_all_objects
    )
    ...
    extract_postgres = PythonOperator(
        task_id="extract_postgres", python_callable=postgresql_ingestion.postgres_to_s3
    )
    ...
    (
        [extract_s3, extract_google_sheets, extract_postgres]
        >> dbt_run
        >> dbt_test
        >> dbt_docs
    )
